...
export function createPeriodicReviewsDatabaseReadinessProbe(database: DatabaseProbe): ReadinessProbe {
  return {
    async check() {
      ...
      const unscoped = await database.query(
        `SELECT count(*)::int AS visible_reviews FROM ${periodicReviewTable}`,
      );
      return unscoped.rows.length === 1 && unscoped.rows[0].visible_reviews === 0;
    },
  };
}
