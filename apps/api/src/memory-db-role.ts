...
const lockName = "life-os-memory-role-v1";

export class MemoryDbRoleError extends Error {
  constructor(message: string, readonly code: "CONFIGURATION_INVALID" | "MIGRATIONS_PENDING" | "BASE_ROLE_NOT_READY" | "TABLE_NOT_READY" | "APPLY_FAILED") {
    super(message); this.name = "MemoryDbRoleError";
  }
}

export type MemoryDbRoleConfiguration = ApplicationDbRolePlanConfiguration;
export const memoryDbRoleConfigurationFromEnv = applicationDbRolePlanConfigurationFromEnv;

export interface MemoryDbRolePlan {
  roleName: string;
  schemaName: string;
...
export async function planMemoryDatabaseRole(pool: Pool, roleName: string) {
...
async function quoted(client: PoolClient, roleName: string) {
  ...
  if (!result.rows[0]) throw new MemoryDbRoleError("Unable to quote capability", "APPLY_FAILED");
  return result.rows[0];
}

export async function applyMemoryDatabaseRole(pool: Pool, roleName: string) {
  const migrations = await planDatabaseMigrations(pool);
  if (migrations.pending.length) throw new MemoryDbRoleError("Migrations pending", "MIGRATIONS_PENDING");
  const baseline = await planApplicationDatabaseRole(pool, roleName);
  if (!baseline.ready) throw new MemoryDbRoleError("Baseline role is not ready", "BASE_ROLE_NOT_READY");
  return withClient(pool, async (client) => {
    ...
    try {
      ...
      if (!before.tableExists || !before.forcedRls || !before.nonOwner) {
        throw new MemoryDbRoleError("Memory table is not protected", "TABLE_NOT_READY");
      }
      const value = await quoted(client, baseline.roleName);
      await client.query("BEGIN");
      try {
        await client.query(`REVOKE ALL PRIVILEGES ON TABLE ${value.schema_ident}."${memoryItemTable}" FROM ${value.role_ident}`);
        await client.query(`GRANT SELECT, INSERT, UPDATE ON TABLE ${value.schema_ident}."${memoryItemTable}" TO ${value.role_ident}`);
        const after = await inspect(client, baseline.roleName, [], true);
        if (!after.ready) throw new MemoryDbRoleError("Least privilege check failed", "TABLE_NOT_READY");
...
export async function revokeMemoryDatabaseRole(pool: Pool, roleName: string) {
  const baseline = await planApplicationDatabaseRole(pool, roleName);
  if (!baseline.roleExists) throw new MemoryDbRoleError("Baseline role missing", "BASE_ROLE_NOT_READY");
  return withClient(pool, async (client) => {
    const value = await quoted(client, baseline.roleName);
    const relation = await client.query<{ exists: boolean }>(
      `SELECT to_regclass(format('%I.%I', current_schema(), '${memoryItemTable}')) IS NOT NULL AS exists`,
    );
