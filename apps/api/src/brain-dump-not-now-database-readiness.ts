...
const unscopedSql = `
SELECT
  (SELECT count(*)::int FROM ${brainDumpClassificationTable}) AS visible_classifications,
  (SELECT count(*)::int FROM ${notNowItemTable}) AS visible_not_now_items
`;

export function createBrainDumpNotNowDatabaseReadinessProbe(
  ...
  return {
    async check() {
      ...
      return unscoped.rows.length === 1
        && unscoped.rows[0].visible_classifications === 0
        && unscoped.rows[0].visible_not_now_items === 0;
    },
  };
}
